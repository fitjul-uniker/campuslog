...
type StrandsProps = Partial<StrandRuntimeProps> & {
  className?: string;
  style?: CSSProperties;
};
...
export function Strands({
  ...
  const propsRef = useRef<StrandRuntimeProps>({
    colors,
    count,
    speed,
    amplitude,
  ...
  useEffect(() => {
    ...
    // Props are read through propsRef so the WebGL context is created only once.
  ...
  return (
    <div
      ref={containerRef}
      className={`strands-container${className ? ` ${className}` : ""}`}
      style={style}
      aria-hidden="true"
    />
  );
