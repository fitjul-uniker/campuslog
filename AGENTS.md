...
Codex는 작업 전에 아래 순서로 프로젝트 기준을 확인합니다. 문서와 코드가 다르면 이 우선순위로 판단하고, 불일치가 큰 경우 바로 구현하지 말고 사용자에게 알립니다.
...
5. **검증 후 보고**
   ...
   - 수정한 파일, 변경 이유, 확인한 문서, 남은 작업을 요약합니다.

## MVP Scope

이 범위는 1차 MVP 기준입니다. 1차 MVP에서 집중할 기능은 아래 3개입니다.
...
- 팀 작업 기록이 필요한 변경은 `docs/TASK_LOG.md`와 `docs/WORK_STATUS.md` 갱신을 검토합니다. 단, 사용자가 특정 파일만 요청한 경우 먼저 변경 범위를 확인합니다.
