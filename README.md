...
프로젝트, 공모전, 인턴, 대외활동 등 다양한 경험을 제목, 기간, 역할, 내용, 성과, URL과 설명으로 구성된 관련 링크 중심으로 기록합니다.
...
CampusLog의 프론트엔드 디자인은 핵심 MVP 흐름을 지키면서 가능한 최고 수준의 UI/UX를 목표로 합니다. 디자인 표현 방식에는 불필요한 제한을 두지 않으며, Three.js, React Three Fiber, GSAP, Framer Motion, WebGL, shader, Lottie, particle, glassmorphism, morphing animation, scroll-based animation, micro interaction, premium landing experience 같은 최신 웹 기술을 사용할 수 있습니다.

판단 기준은 "과한 효과를 피한다"가 아니라 "사용자 경험을 높이면서 성능, 접근성, 유지보수성을 지키는가"입니다. Core Web Vitals, 모바일 안정성, reduced motion, 텍스트 대비, 키보드 접근성을 크게 해치지 않는 범위에서 프리미엄 제품 수준의 인터랙션과 시각적 완성도를 추구합니다.

현재 프론트엔드는 `/`에서 React Three Fiber 기반 인터랙티브 3D 노트 표지를 보여주고, 노트를 선택하면 `/dashboard`의 경험 목록과 핵심 기능으로 이동합니다. 3D 표지는 `three`, `@react-three/fiber`, `@react-three/drei`를 사용하며 정적 이미지가 먼저 노출되지 않는 로딩 전환, 제한된 DPR, `prefers-reduced-motion` 처리를 포함합니다.
