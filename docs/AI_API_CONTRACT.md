...
3. 답변 초안 생성 — 구현 완료
   - 자기소개서 300자 / 500자 / 1000자 프리셋 또는 100~2000자 직접 입력과 면접·JD·기타 목적별 결과 중 1개 버전
   - 직접 입력 자기소개서는 `customCharacterCount`를 서버에서 검증하고 결과 draft의 `targetCharacterCount`에 저장해 스트리밍·분량 교정·재조회에 같은 제한을 사용
