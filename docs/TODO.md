...
- 2026-08-25: 오늘의 기록 Data Grid 압정을 전용 `dashboard_activity_pins` table·본인 RLS·활동 FK cascade와 연결하고, 고정 시각 상단 정렬·낙관적 토글·실패 rollback·재활성화 재조회를 구현. 원격 Supabase migration과 REST table·column 확인을 완료했으며 로그인 저장·새로고침 UI smoke test가 남음 (`ISSUE-201`).
- 2026-08-20: 실제 구현 기준 `docs/testing/`에 계획·35개 TC·최신 결과·누적 이력을 구축하고 자동화, 인증·RLS, 실제 DB CRUD, private PDF 업로드·개별 삭제, 390px 반응형과 OpenAI 추천·분석·답변 초안의 최소 호출 및 저장·재조회를 검증. 잘못된 로그인에서 Server Action runtime overlay가 노출되던 `ISSUE-161`을 최소 수정하고 회귀 테스트를 추가. 최종 집계는 PASS 32, FAIL/BLOCKED/NOT_RUN 0, NOT_IMPLEMENTED 3이며 Node 186개·lint·typecheck·production build가 통과했다.
...
- 2026-08-25: 이미지 단독 자기소개서 추천 결과는 OCR 전체 문장 대신 60자 이하 추출 의도, 핵심 키워드 최대 3개, 고정 기본 제목 순서로 자연스러운 대표 제목을 표시. 원 질문·저장 schema·텍스트 및 혼합 입력·긴 JD 규칙은 유지하고 사용자가 직접 로직 테스트 완료 (`ISSUE-202`).
- 2026-08-11: 추천 결과와 추천 기록에서 240자를 넘는 긴 JD를 대표 제목으로 노출하지 않고 짧은 분석 제목과 기본 접힘 `입력한 JD 보기`로 분리. 짧은 텍스트 자기소개서·면접 질문은 기존 제목을 유지하고, 펼친 원문은 내부 스크롤로 제한. 사용자가 직접 로직 테스트 완료 (`ISSUE-145`).
...
- 2026-08-10: 자기소개서 답변 생성에 기존 300자 / 500자 / 1000자와 함께 100~2000자 `직접 입력`을 추가. 700자·1500자 등 기업별 최대 글자 수를 입력하면 약 88~95% 분량으로 생성·교정하고 목표 제한을 API 요청·저장 초안·대기 및 결과 화면에 유지하도록 확장. 사용자가 이번 변경의 직접 로직 테스트를 완료 (`ISSUE-142`).
- 2026-08-10: 계정 메뉴에 기존 디자인을 유지한 닉네임 수정 dialog를 추가. 가입과 같은 1~20자 검증을 서버에서 적용하고 `user_metadata.campuslog_profile`의 이름·완료 시각·provider metadata를 보존한 채 닉네임만 저장하며, 성공 시 데스크톱·모바일 프로필 표시를 즉시 동기화. 사용자가 실제 로그인 환경에서 닉네임 저장과 화면 반영·새로고침 후 유지를 직접 로직 테스트로 확인 (`ISSUE-141`).
...
- [x] 로그인 AI API의 malformed JSON·본문/입력 상한·최소 근거·runtime-local rate guard를 OpenAI 호출 전에 차단하는 통합 검증 (`TC-028`)
...
- [x] 실제 로그인 경험 1건의 AI 분석 결과 구조·DB 저장·새로고침 재조회 검증 (`TC-022`)
...
- [x] 대표 자기소개서 분량 1건의 답변 초안 생성 구조·저장·재조회 검증 (`TC-027`)
- [x] AI 실패·취소 시 원본·마지막 유효 결과 보존을 live 장애 없이 mock/fault injection으로 검증 (`TC-023`)
- [x] 기록 보완 루프 UX 개편: 부족 정보 카드 안에서 바로 답변 저장, 질문 생성 단계 제거, 추천 즉시 반영 (`ISSUE-078`)

### Medium

- [x] 기존 AI 작업 중 UI를 보존한 채 경험 분석·추천·활동 종료 합성·답변 초안을 내부 route 이동 가능한 background task와 compact 완료·실패 알림에 연결 (`ISSUE-143`)
- [x] 백그라운드 AI 추천 완료 `결과 보기`를 방금 저장된 추천 기록 상세에 직접 연결 (`ISSUE-144`)
- [x] 긴 JD 추천 결과를 짧은 대표 제목과 접힌 원문 disclosure로 분리해 추천 기록의 읽기 흐름 보호 (`ISSUE-145`)
- [x] 이미지 단독 자기소개서 추천 결과의 OCR 원문 제목을 짧은 의도·핵심 키워드 기반 제목으로 정리하고 직접 로직 테스트 완료 (`ISSUE-202`)
...
- [x] 답변 초안 `사용된 근거`를 추천 match의 매칭 근거 문장만 남기는 단일 제목 목록으로 정리하고 경험 원문·반복 접두 제목 제거 (`ISSUE-220`)
- [x] 오늘의 기록 활동 현황을 모바일에서도 데스크톱과 같은 머리글·열 순서·단일 행 table로 유지하고 부족한 폭은 접근 가능한 Grid 내부 스크롤로 제공 (`ISSUE-219`)
- [x] 백그라운드 AI compact pending 상태에 실제 서버 상태와 작업별 대기 문장의 2.4초 순환을 적용하고 반복 낭독 없이 현재 상태만 보조기술에 전달 (`ISSUE-218`)
- [x] 독립 AI 분석의 헤더 hairline→첫 `경험 요약` 간격을 이후 구획과 같은 22~23px 리듬으로 통일 (`ISSUE-217`)
- [x] 나의 활동 완료 경험 상세의 역할 제목·본문 타이포와 여백을 활동 내용·성과와 통일 (`ISSUE-216`)
- [x] 독립 AI 분석 결과 헤더 아래 이중 구분선을 모든 화면 폭에서 단일 1px hairline으로 정리 (`ISSUE-215`)
- [x] 독립 AI 분석 헤더를 `활동 경험 상세` 단일 복귀 액션으로 줄이고 기존 두 번째 버튼 위치·모바일 H1 우측 상단 44px 배치 유지 (`ISSUE-214`)
- [x] 독립 완료 경험 상세의 모바일 `나의 활동` 복귀 액션을 H1 우측 상단 44px capsule로 유지하고 상세 Glass 겹침·가로 overflow 제거 (`ISSUE-213`)
- [x] 모바일 상단 앱 바의 `CampusLog AI`에서 `CampusLog`를 PC 메뉴와 같은 브랜드 워드마크체로 표시하고 44px 터치 높이·390px overflow 보호 (`ISSUE-212`)
- [x] CampusLog AI 추천 헤더의 모바일 `추천 기록`에 18px History 아이콘과 문구를 함께 표시하고 44px 높이·390px 제목 무겹침 유지 (`ISSUE-211`)
- [x] CampusLog AI 추천 화면 설명을 의미를 유지한 짧은 문장으로 정리해 모바일 한 줄 표시 검증 (`ISSUE-210`)
- [x] 나의 활동 모바일 완료 경험 상세 액션의 좌우 inset을 맞추고 `상세 보기 / 수정·삭제 / AI 분석` 대칭 행 구성 적용 (`ISSUE-209`)
- [x] 모바일 상단 앱 바에 `오늘의 기록 / 나의 활동 / CampusLog AI` 원래 메뉴명을 표시하고 CampusLog 브랜드 서체는 후속 `ISSUE-212`에서 PC와 통일 (`ISSUE-208`)
- [x] 오늘의 기록 캘린더 모바일 헤더의 이전·오늘·다음 control group을 연·월 Select와 같은 행의 우측 상단에 배치하고 390px overflow·44px 터치 높이 검증 (`ISSUE-207`)
- [x] 추천 경험의 답변 초안을 Liquid Glass segmented 형식 선택과 단일 생성·미리보기·복사 작업면으로 정돈하고 자동화 검증·빈 추천 상태 route 확인 완료 (`ISSUE-206`)
- [ ] 추천 가능한 경험이 있는 로그인 세션에서 답변 초안의 생성 전·후·복사·다시 만들기 시각 smoke test 수행 (`ISSUE-206` 후속 검증)
- [x] 자기소개서 `직접 입력`의 큰 설정 표면을 한 줄 `최대 글자 수`·44px Liquid Glass 숫자 control·짧은 예상 분량으로 축약 (`ISSUE-205`)
- [x] 추천 Top 3의 기존 Accordion·정보 항목·순서를 유지한 채 본문 행간·구획 여백·근거 hairline을 완화하고 `직접 입력` 숫자 control·focus·invalid 표현을 현대화 (`ISSUE-204`)
- [x] 인증 후 3D 책 표지에 별도 시각 문구 없이 책 정중앙까지 이동하는 macOS형 포인터를 추가하고 12초 중 약 8초 숨김, 중앙 클릭 모션, reduced-motion 정지, 전체 책 링크 접근성 이름을 적용 (`ISSUE-203`)
- [x] stale 개발 서버가 최신 HTML이 참조한 `layout.css`를 404로 반환해 기본 하이퍼링크 화면이 된 재발 상황을 단일 서버 재시작으로 복구 (`ISSUE-200`)
- [x] 시각적 초기 skeleton은 오늘의 기록·나의 활동·CampusLog AI에만 유지하고 그 밖의 화면은 `sr-only` `LoadingStatus`로 안내 (`ISSUE-199`)
- [x] CampusLog AI root pending에서는 시각 skeleton을 생략하고 서버 경험 count 판별 뒤 정확한 본문 skeleton 한 개만 유지 (`ISSUE-198`)
- [x] 제품 워드마크 클릭 시 출발 route skeleton을 차단하고, 책 표지 전용 시각 skeleton은 후속 범위 결정에서 제거 (`ISSUE-197`, `ISSUE-199`)
- [x] CampusLog AI route skeleton의 Grid row stretch를 제거하고 primary-page 레일·헤더·액션·본문 좌표를 완료 화면과 일치 (`ISSUE-196`)
- [x] CampusLog AI 추천 진입 전 서버 RLS 경험 count로 빈 상태형·활동 보유 폼형 skeleton을 정확히 선택 (`ISSUE-195`)
- [x] CampusLog AI 새로고침 시 초기 skeleton과 완료 빈 상태 탭의 외곽 위치·폭·높이·곡률·padding 일치 (`ISSUE-194`)
- [x] 전체 화면용 Liquid Skeleton 기반을 구현한 뒤 시각 적용 범위를 세 주 화면으로 축소하고 나머지는 보조 기술 안내만 유지 (`ISSUE-193`, `ISSUE-199`)
- [x] 나의 활동 완료 경험 상세의 첨부 구획·파일 행을 관련 링크와 같은 평평한 Liquid 스타일로 통일 (`ISSUE-192`)
- [x] 경험 폼 활동기간 그룹 제목과 시작·종료 보조 label의 시각 위계 분리 (`ISSUE-191`)
- [x] 새 경험 폼의 다중 행 역할·활동 내용 label·첨부 간격·관련 링크 정렬/capsule·단색 파일/저장·안정된 hover와 작성/수정 상세형 내부 scroll 통일 (`ISSUE-190`)
- [x] 나의 활동 빈 목록의 높이를 패널과 일치시키고 오해 가능한 중앙 `+` 제거·과거 경험 이동 Glass capsule 적용 (`ISSUE-189`)
- [x] 오늘의 기록 활동 현황 빈 상태를 정확한 진행 활동 문구로 정리하고 데이터 로딩 실패를 비침해적 Glass 팝업으로 전환 (`ISSUE-188`)
- [x] 로그인·회원가입 첫 카드를 공통 Liquid Glass 양식으로 통일하고 별도 가입 방식 선택 단계를 제거 (`ISSUE-187`)
- [x] 활동 추가의 이중 전체 화면 surface·과한 확장 motion을 제거하고 두 작성 탭의 90% 밝은 표면을 통일 (`ISSUE-186`)
- [x] 활동 추가·날짜별 기록 `+` 작성 화면을 단일 Glass 재질·반투명 control·capsule action으로 통일 (`ISSUE-185`)
- [x] 새로고침 CSS 미적용 개발 캐시를 복구하고 연속 새로고침 회귀 확인 (`ISSUE-184`)
- [x] 삭제 글자·아이콘의 빨강과 이동·중립·삭제 capsule의 얕은 액션 그림자를 통일 (`ISSUE-183`)
- [x] 삭제 버튼과 colorful AI 분석 버튼도 의미 색상·gradient animation을 유지한 44px capsule로 통일 (`ISSUE-182`)
- [x] 화면 이동과 중립 저장·수정·완료 버튼을 추천 기록의 흰 Glass capsule로 통일하고 색상 의미 버튼은 제외 (`ISSUE-181`)
- [x] 추천·추천 기록·상세·수정·분석의 우측 상단 보조 액션을 헤더 상단·우측 끝·44px 기준으로 통일 (`ISSUE-180`)
- [x] 경험 수정의 헤더·복귀 버튼·본문·하단 액션 기준선과 intro 연동·고정 하단·내부 scroll을 경험 상세와 통일하고 중복 `파일 추가` 제거 (`ISSUE-179`)
- [x] 추천 이미지 빈 첨부 영역을 활동 수정 첨부와 같은 양식으로 통일하고 선택 버튼을 아이콘 없는 `파일 선택`으로 변경 (`ISSUE-178`)
- [x] 추천 기록을 나의 활동과 같은 primary-page·차콜 Gooey 검색·master-detail·transient scrollbar 구조로 통일하고 우측 상세 중복 kicker 제거 (`ISSUE-177`)
- [x] 독립·스플릿 AI 분석 결과 헤더에서 중복 `AI 경험 분석 결과` kicker를 제거하고 생성일·대표 제목만 유지 (`ISSUE-176`)
- [x] 나의 활동 목록과 독립 경험 상세·AI 분석의 Breadcrumb·H1·패널 최초 기준선을 통일하고 고정 scroll range·stable gutter·24px track inset으로 스크롤바를 안정화 (`ISSUE-175`)
- [x] 독립 경험 상세·AI 분석의 하단 곡률을 복원하고 마지막 액션 아래 30px 안전 여백을 실제 scroll range에 포함 (`ISSUE-174`)
- [x] 독립 경험 상세·AI 분석의 도입부와 패널을 시각적으로 분리하고 함께 스크롤하되 패널 하단은 항상 좌측 메뉴 하단에 고정 (`ISSUE-173`)
- [x] 독립 경험 상세·AI 분석의 외곽/내부 스크롤을 분리하고 좌측 메뉴 하단 정렬, 12px 버튼, 상단 `나의 활동`, 삭제 아이콘 색, 분석 중복선을 통일 (`ISSUE-172`)
- [x] 독립 완료 경험 상세를 공통 콘텐츠 레일 전체 폭·단일 내부 여백·좌측 메뉴 하단 정렬 구조로 통일 (`ISSUE-171`)
- [x] 독립 완료 경험 상세를 Breadcrumb·나의 활동 H1·설명·상세 H2의 공통 sub-page 위계로 통일 (`ISSUE-170`)
- [x] 나의 활동 진행 중 상세를 완료 경험과 같은 내부 스크롤·안전 여백 구조로 통일하고 날짜별 기록 `+`를 차콜 원형 CTA로 변경 (`ISSUE-169`)
- [x] 활동 현황·활동 상세·나의 활동 진행 목록/상세의 상태색 capsule을 통일하고 AI 초안·경험 수정 폼의 중복 구분선·설명과 관련 링크 legend 정렬 정리 (`ISSUE-168`)
- [x] 나의 활동 AI 분석 결과 패널의 첫 viewport 하단을 좌측 메뉴와 정렬하고 외곽/내부 스크롤 viewport 분리 및 scrollbar 모서리 안전 여백 적용 (`ISSUE-167`)
- [x] 오늘의 기록 활동 상태 목록을 Liquid Glass Data Grid로 통합하고 pin·상세·수정·삭제·고정 5행 페이지네이션과 모바일 카드 행 적용 (`ISSUE-166`)
- [x] 나의 활동 첫 화면의 master-detail 목록·상세 하단을 좌측 메뉴 하단과 정렬 (`ISSUE-165`)
- [x] 활동 상세 설명을 액션과 분리해 요약 패널 첫 행으로 이동하고 긴 설명의 액션 줄바꿈 방지 (`ISSUE-164`)
- [x] 오늘의 기록 진행 활동 상세 초기 로딩의 과거 크림색 shimmer 카드 제거 및 접근성 안내 통일 (`ISSUE-163`)
- [x] 오늘의 기록 캘린더를 12개월 미래까지 열고 진행 중·시작 예정 활동 기간 안의 계획 작성·표시와 완료 경험 합성 제외를 적용하고 사용자 직접 로직 테스트 완료 (`ISSUE-162`)
- [x] 나의 활동 우측 상세의 외곽 표면·내부 스크롤 viewport 분리와 상하 30px 안전 영역 적용 (`ISSUE-160`)
- [x] 나의 활동 스크롤바의 페이지 2px·패널 3px 인셋 위계와 650ms transient 상태 적용 (`ISSUE-159`)
- [x] 나의 활동 선택의 목록 폭 축소·상세 진입 master-detail 모션과 reduced motion 적용 (`ISSUE-158`)
- [x] 나의 활동 기본 목록 패널을 오늘의 기록·추천 입력과 같은 공통 콘텐츠 폭으로 통일 (`ISSUE-157`)
- [x] 독립 AI 분석의 재분석 액션을 결과 표면 내부 footer로 통합 (`ISSUE-156`)
- [x] 비교용 페이지 로딩 탭 제거 및 접근성 안내만 유지 (`ISSUE-155`)
- [x] AI 처리 overlay의 중복 안내 문장 제거와 추천 Top 3별 `활동 보기` 이동 제공 (`ISSUE-152`)
- [x] 경험 수정 폼 관련 링크 `URL` 제목의 왼쪽 기준선 정렬 (`ISSUE-153`)
- [x] AI 처리 문구 목록 갱신 시 첫 문장 중복 재생 방지 (`ISSUE-151`)
- [x] 나의 활동 `즐겨찾기`·`모든 활동` 구획 제목의 글자 scale 없이 위치만 전환 (`ISSUE-150`)
- [x] 나의 활동 상세의 우측 퇴장 완료 후 목록 확장 적용 (`ISSUE-149`)
- [x] 오늘의 기록·나의 활동·CampusLog AI의 첫 로딩에서 모든 시각적 placeholder를 제거하고 앱 창·페이지 헤더만 유지 (`ISSUE-148`)
- [x] 나의 활동 상세의 활동 기간과 역할을 각각 전체 폭의 독립 행으로 분리 (`ISSUE-147`)
