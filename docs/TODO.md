...
- [x] 오늘의 기록 활동 현황을 모바일에서도 데스크톱과 같은 머리글·열 순서·단일 행 table로 유지하고 부족한 폭은 접근 가능한 Grid 내부 스크롤로 제공 (`ISSUE-215`)
- [x] 백그라운드 AI compact pending 상태에 실제 서버 상태와 작업별 대기 문장의 2.4초 순환을 적용하고 반복 낭독 없이 현재 상태만 보조기술에 전달 (`ISSUE-214`)
- [x] 독립 AI 분석의 헤더 hairline→첫 `경험 요약` 간격을 이후 구획과 같은 22~23px 리듬으로 통일 (`ISSUE-213`)
- [x] 나의 활동 완료 경험 상세의 역할 제목·본문 타이포와 여백을 활동 내용·성과와 통일 (`ISSUE-212`)
- [x] 독립 AI 분석 결과 헤더 아래 이중 구분선을 모든 화면 폭에서 단일 1px hairline으로 정리 (`ISSUE-211`)
- [x] 독립 AI 분석 헤더를 `활동 경험 상세` 단일 복귀 액션으로 줄이고 기존 두 번째 버튼 위치·모바일 H1 우측 상단 44px 배치 유지 (`ISSUE-210`)
- [x] 독립 완료 경험 상세의 모바일 `나의 활동` 복귀 액션을 H1 우측 상단 44px capsule로 유지하고 상세 Glass 겹침·가로 overflow 제거 (`ISSUE-209`)
- [x] 모바일 상단 앱 바의 `CampusLog AI`에서 `CampusLog`를 PC 메뉴와 같은 브랜드 워드마크체로 표시하고 44px 터치 높이·390px overflow 보호 (`ISSUE-208`)
- [x] CampusLog AI 추천 헤더의 모바일 `추천 기록`에 18px History 아이콘과 문구를 함께 표시하고 44px 높이·390px 제목 무겹침 유지 (`ISSUE-207`)
- [x] CampusLog AI 추천 화면 설명을 의미를 유지한 짧은 문장으로 정리해 모바일 한 줄 표시 검증 (`ISSUE-206`)
- [x] 나의 활동 모바일 완료 경험 상세 액션의 좌우 inset을 맞추고 `상세 보기 / 수정·삭제 / AI 분석` 대칭 행 구성 적용 (`ISSUE-205`)
- [x] 모바일 상단 앱 바에 `오늘의 기록 / 나의 활동 / CampusLog AI` 원래 메뉴명을 표시하고 CampusLog 브랜드 서체는 후속 `ISSUE-208`에서 PC와 통일 (`ISSUE-204`)
- [x] 오늘의 기록 캘린더 모바일 헤더의 이전·오늘·다음 control group을 연·월 Select와 같은 행의 우측 상단에 배치하고 390px overflow·44px 터치 높이 검증 (`ISSUE-203`)
