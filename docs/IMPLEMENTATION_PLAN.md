...
- `feature/ai-analysis-v2`에서 `/api/analyze` structured output과 prompt를 v2로 확장하고, STAR, 원본 근거, 부족 정보, 자소서 소재 각도, 역량별 근거를 타입 / 저장소 / Supabase migration / 분석 결과 화면에 연결했습니다. 이후 `feature/ai-analysis-gap-answers`에서 신규 분석 출력과 화면을 v2.1로 간소화해 요약, STAR, 주요 성과, 부족 정보 답변, 키워드만 표시합니다. 기존 분석 네 필드와 v2 확장 필드는 유지하고 v1 / 과거 v2 저장 결과는 기본값으로 보정해 읽습니다.
...
- `feature/ai-evidence-followup`에서 `/api/evidence-followups` structured output과 prompt를 추가하고, 분석 v2 / 추천 v2 / 답변 초안에서 드러난 부족 근거를 안전한 보완 질문으로 바꿉니다. 사용자 답변은 원본 경험 필드를 자동 수정하지 않고 별도 `experience_followups` table 및 `campuslog:v1:experience-followups` localStorage key에 저장합니다. `feature/ai-analysis-gap-answers`부터 분석 부족 정보는 별도 질문 생성 버튼 없이 카드 안에서 바로 답변하며, 보완 답변은 새 추천 / 답변 초안 입력에 즉시 반영됩니다.

1. AI 경험 분석 v2.1 — 구현 완료
   - 기존 `summary`, `competencyTags`, `achievements`, `keywords` 호환 필드는 유지하되 신규 출력은 요약, STAR, 주요 성과, 부족 정보 답변, 키워드 중심으로 간소화합니다.
   - 분석 결과의 부족 정보는 `id`, `category`, `title`, `reason`, `question`, `answer`, `answeredAt` 또는 `updatedAt`을 갖는 카드 단위로 저장합니다.
   - 핵심 역량 태그, 역량별 근거, 원본 근거, 자소서 소재 각도는 신규 분석에서 생성하지 않고 화면에도 표시하지 않습니다.
   - 업데이트 필요 상태는 원본 경험이 분석 이후 수정된 경우에만 표시하고, 보완 답변만 추가된 경우에는 추천 / 답변 초안 입력에 즉시 반영합니다.
2. 추천 v2 — 구현 완료
...
4. 기록 보완 루프 — 구현 완료
   - AI가 부족한 정보를 질문 형태로 제안하고, 사용자가 분석 카드 안에서 바로 답하면 별도 보완 답변으로 저장합니다.
   - 보완 답변이 들어와도 기존 분석이 있던 경험을 `needs_reanalysis`로 강제하지 않습니다. 사용자가 명시적으로 재분석하면 원본 경험 + answered followup으로 분석 v2.1을 재생성합니다.
...
5. AI 경험 분석 v2.1: 요약, STAR, 주요 성과, 부족 정보 답변, 키워드 — 구현 완료
6. 추천 v2: 문항 / JD 요구사항 추출, 경험 Top 3 매칭, 부족 근거 표시 — 구현 완료
7. 답변 초안 생성: 500자 / 800자 / 1000자 자기소개서 + 면접 + 포트폴리오 버전 — 구현 완료
8. 기록 보완 루프: 부족 정보 카드 내 사용자 답변 저장, 추천 즉시 반영, 명시적 재분석 — 구현 완료
