...
현재 프로젝트는 `feature/experience-crud` 브랜치에서 localStorage 기반 경험 CRUD 구현과 검증을 완료하고 PR 준비 중인 상태입니다. 이 브랜치는 아직 `main`에 merge 전입니다. 다음 구현 단계는 `feature/ai-analysis` 범위의 AI 분석 API Route와 분석 결과 저장 흐름입니다.
...
현재 구현은 `feature/experience-crud` 단계입니다. 경험 저장 / 수정 / 삭제는 localStorage 기준으로 동작하며, AI 분석 요청과 AI 추천 요청은 아직 실제 API와 연결되지 않았습니다.
...
- [ ] `feature/experience-crud` PR 생성, 리뷰, merge
- [ ] `feature/ai-analysis` 브랜치에서 `/api/analyze` 구현
- [ ] 활동 경험 상세 화면에서 AI 분석 요청 CTA 구현
- [ ] 분석 결과 저장 후 `analysisStatus: "analyzed"` 상태 전환 연결
- [ ] 분석 결과가 있는 경험 수정 시 `재분석 필요` UI 흐름 확인
- [ ] Vercel 배포 준비
