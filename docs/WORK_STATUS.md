...
현재 프로젝트는 `fix/dashboard-logo-overflow` 작업 브랜치에서 제품 화면 공통화, 대시보드·추천 기록의 목록-상세 인터랙션, 관련 링크 URL·설명 입력과 파비콘 표시를 구현하고 게시 전 통합 리뷰와 로컬 검증까지 통과한 상태입니다. `/`의 3D 표지는 유지하고, 나머지 제품 라우트는 같은 흰색 앱 셸을 공유합니다. AI 분석·추천 흐름은 유지하며 경험 저장은 기존 v1 링크를 보존하는 `campuslog:v2:experiences` 마이그레이션을 사용합니다. 최신 Organization `main`을 반영한 원격 브랜치와 Draft PR #26을 생성해 팀 리뷰를 기다리고 있습니다. MVP 검증은 아직 완료하지 않았고, 새 링크를 실제 저장한 뒤 상세·새로고침까지 확인하는 수동 테스트는 `ISSUE-021`로 남아 있습니다.
...
2026-07-10 `design/dashboard-polish` 기준으로 메인 대시보드의 CTA 위계, 빈 상태, 목록 로딩 skeleton, 실패 Alert, 경험 카드 정보 구조, 분석 상태 배지, 정렬/필터 컨트롤 UI 배치를 정리했습니다. 이 카드·정렬/필터 UI는 2026-07-12 제목 전용 목록과 검색 구조로 대체되었으며, storage 구조, localStorage key, API Route, AI 요청 방식, 경험 CRUD 로직은 유지됩니다.

2026-07-10 `design/responsive-brand-polish` 기준으로 Campus Green + Mint 브랜드 토큰, focus ring, hover 상태, 데스크톱 좌측 사이드바 / 모바일 상단 App Bar 레이아웃, 모바일 CTA / 카드 / EmptyState / 상세 화면 줄바꿈을 정리했습니다. 당시의 Lucide `BookMarked` 아이콘 후보는 2026-07-11 사용자 요청에 따라 CampusLog 텍스트 워드마크 favicon / app icon으로 교체했습니다. storage 구조, localStorage key, API Route, AI 요청 방식, 경험 CRUD 로직은 변경하지 않았습니다.
...
2026-07-11 `feature/interactive-notebook-ui-review` 기준으로 대시보드의 CampusLog 워드마크와 공통 책 프레임을 정리하고, 새 경험 작성은 우측 하단 CTA 하나로 통일했습니다. Fork 커밋 이력 없이 작업 트리 내용만 옮긴 뒤 PR #23의 merge commit `23492c1`로 Organization `main`에 반영했습니다. 당시 데스크톱 좌측 메뉴에는 포인터 근접 이동과 비활성 메뉴의 미세 blur를 적용했습니다. 제품 화면의 책 프레임은 2026-07-12 흰색 공통 앱 셸로 대체했고 `/`의 3D 표지만 유지하며, 모바일에는 같은 핵심 메뉴를 추가하고 작은 글자의 blur는 제거했습니다.
...
2026-07-11 `codex/fix-experience-list-scroll` 기준으로 `/dashboard`의 고정 높이 책 프레임에서 경험 목록이 첫 카드 아래로 스크롤되지 않던 문제를 수정했습니다. 오른쪽 페이지 높이 체인을 보강하고 목록 전용 스크롤 영역과 고정 `새 경험 기록하기` 액션 영역을 분리했습니다. 데스크톱 1440×900과 모바일 390×844에서 마지막 카드 접근, 스크롤 중 CTA 좌표 고정, 빈 상태 CTA 노출, 새 경험 작성 화면 이동을 확인했으며 localStorage와 AI 데이터 흐름은 변경하지 않았습니다. 이 수정은 PR #24의 merge commit `84e7f6d`, 후속 문서는 PR #25의 merge commit `55c3ade`로 Organization `main`에 반영되었습니다.

2026-07-12 `fix/dashboard-logo-overflow` 작업 트리에서 `/`을 제외한 제품 라우트를 대시보드와 같은 흰색 SaaS 작업 공간으로 통일했습니다. 대시보드는 제목 전용 활동 목록, CountUp 활동 개수, Gooey 스타일 검색, 목록을 유지하는 인라인 상세로 재구성했습니다. 인라인 상세의 `전체 화면으로 보기`는 같은 정보 구조의 활동 전체 화면으로 이어지며, 미분석·재분석 필요 활동에서는 Border Beam형 AI 분석 요청을 즉시 실행할 수 있습니다.

2026-07-12 추천 기록 화면은 활용 목적·추천 경험·질문을 포함한 상세형 행을 유지하면서 검색과 목록-상세 전환을 적용했습니다. 선택 전에는 목록을 중앙에 두고, 선택 후에는 목록이 왼쪽에 유지된 채 오른쪽 상세가 열립니다. 모바일에서는 공통 상단 메뉴와 목록 위·상세 아래 구조로 전환하며 키보드 초점 복귀와 reduced motion을 지원합니다.

2026-07-12 최종 로컬 검증에서 `npm run lint`, `tsc --noEmit`, `npm run build`, `git diff --check`를 통과했습니다. 브라우저에서 대시보드·새 경험·활동 전체 화면·수정·분석·AI 추천·추천 기록 7개 라우트의 공통 앱 셸, 레거시 책 프레임 제거, 가로 overflow 0, 콘솔 warning/error 0을 확인했습니다. 추천 기록은 현재 저장 데이터가 없어 빈 상태를 실제 확인했고, 저장 기록 선택 상태는 코드·타입·빌드 기준으로 검증했습니다.

2026-07-12 AI 추천 결과가 새로 생성되면 결과 상단으로 자동 스크롤하도록 보강하고, 추천 활동 제목·항목 라벨·답변 본문의 색상과 크기 위계를 분리했습니다. Gooey 검색 아이콘은 이중 패딩으로 축소되지 않도록 20px 고정 크기와 44px 버블을 적용했습니다. 추천 상세의 원본 링크는 `활동`, 분석 결과 링크는 `AI 분석 결과`로 통일했으며, 활동 전체 화면 액션은 `활동 목록 → AI 분석 요청/AI 분석 결과 → 수정 → 삭제` 순서로 재배치하고 수정 버튼의 과도한 확장을 제거했습니다.

2026-07-12 관련 링크 입력을 단일 큰 텍스트 영역에서 URL·설명 반복 행으로 변경했습니다. 새 링크는 최대 10개까지 개별 추가·삭제할 수 있고 URL 정규화, 중복·형식 오류, 설명만 입력한 상태를 행 단위로 검증합니다. 상세에서는 설명·전체 URL·파비콘을 함께 표시하며, 공개 도메인만 고정 파비콘 제공처에 전달하고 비공개 호스트·IP 주소·실패 응답은 링크 아이콘으로 대체합니다. 기존 `campuslog:v1:experiences`의 문자열 링크 배열은 개수와 URL이 아닌 증빙 메모까지 보존한 채 `campuslog:v2:experiences`의 객체 배열로 자동 변환하고, 완료 marker로 삭제·수정 전 v1 데이터의 재등장을 막습니다. AI 분석·추천 API는 오래 열린 탭의 v1 payload와 새 v2 payload를 모두 받아 정규화합니다.
...
- 경험 목록 대시보드, 빈 상태, 제목 전용 활동 목록 구현
- 활동 경험 작성 / 수정 공통 폼 구현
...
- 활동 관련 링크의 URL·설명 반복 행, 최대 10개 추가·개별 삭제·행 단위 검증 적용
- 관련 링크 상세의 설명·전체 URL·파비콘과 `Link2` fallback 표시
- `campuslog:v1:experiences` 문자열 링크 보존형 v2 마이그레이션과 완료 marker 적용
- AI 분석·추천 API의 v1 문자열 링크·v2 객체 링크 입력 호환 처리
...
- 대시보드 CTA 위계와 제목 목록-상세 정보 구조 정리
...
- CampusLog 텍스트 워드마크 기반 `favicon.svg`, `app-icon.svg` 적용
- `/`을 제외한 모든 제품 라우트의 흰색 공통 앱 셸 적용
- 대시보드 제목 전용 Animated List와 목록-상세 인터랙션 적용
- React Bits 레퍼런스 기반 CountUp 활동 개수 적용
- Gooey 스타일 활동·추천 기록 검색 적용
- 인라인 상세와 활동 전체 화면의 공통 상세 컴포넌트 적용
- 미분석·재분석 필요 활동의 Border Beam형 AI 분석 요청 적용
- 추천 기록 상세형 목록과 목록을 유지하는 우측 상세 적용
- 모바일 공통 상단 내비게이션과 목록-상세 세로 구조 적용

현재 구현은 `fix/dashboard-logo-overflow` 작업 트리에서 정적 검사, 프로덕션 빌드, 브라우저 라우트 검증과 독립 통합 리뷰를 통과하고 Draft PR #26의 팀 리뷰를 기다리는 단계입니다. 경험 저장 / 수정 / 삭제, AI 경험 분석 요청 / 결과 저장, AI 경험 추천 요청 / 결과 저장은 기존 localStorage와 Next.js API Route 흐름을 유지합니다. 대시보드 인라인 상세에서는 기존 분석 API·저장 함수를 재사용해 분석 요청 진입점만 확장했습니다.
...
- [x] PR #24와 후속 문서 PR #25의 Organization `main` 반영 확인
- [x] 대시보드·추천 기록의 실제 콘텐츠 화면을 사용자 관점에서 최종 확인
- [ ] 새 관련 링크를 실제 저장한 뒤 상세 설명·파비콘과 새로고침 유지 수동 확인 (`ISSUE-021`)
- [x] 저장된 추천 기록이 있는 환경에서 추천 목록 선택·닫기·검색을 수동 재검증
- [ ] Draft PR에서 팀 리뷰 진행
- [ ] WebGL 오류 fallback 후속 범위 결정 (`ISSUE-017`)
- [ ] Vercel 배포 준비
