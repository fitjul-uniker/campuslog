...
- [x] 팀 리뷰와 main merge
- [x] 2차 MVP 기능 브랜치 시작
- [x] 답변 초안 `사용된 근거`에서 경험 원문 반복과 문장별 접두 제목을 제거하고 추천 match의 매칭 근거만 표시 (`ISSUE-220`)
- [x] 오늘의 기록 활동 현황의 모바일 카드 변환을 제거하고 데스크톱과 같은 표 머리글·열·62px 행을 유지하며 Grid 내부에만 가로 스크롤 적용 (`ISSUE-219`)
- [x] 백그라운드 AI compact pending 상태에서 경험 분석·추천·활동 합성·답변 초안의 작업별 문구를 2.4초마다 순환하고 실제 서버 상태만 라이브 영역으로 전달 (`ISSUE-218`)
- [x] 독립 AI 분석의 헤더 hairline→첫 경험 요약 제목 간격을 46px에서 22px로 줄여 이후 구획과 통일 (`ISSUE-217`)
- [x] 나의 활동 완료 경험 상세의 역할 제목·본문 간격, 굵기, 글자 크기, 행간과 최대 폭을 활동 내용·성과와 통일 (`ISSUE-216`)
- [x] 독립 AI 분석 결과의 헤더 하단 1px hairline만 유지하고 첫 경험 요약 상단선을 모든 화면 폭에서 제거 (`ISSUE-215`)
- [x] 독립 AI 분석 헤더의 `나의 활동으로 돌아가기`를 제거하고 `활동 경험 상세` 단일 복귀 액션을 기존 데스크톱 위치와 모바일 H1 우측 상단에 유지 (`ISSUE-214`)
- [x] 독립 완료 경험 상세의 모바일 `나의 활동` 복귀 액션을 H1 우측 상단에 유지하고 550px·390px 상세 Glass 무겹침·44px 높이·가로 overflow 없음 확인 (`ISSUE-213`)
- [x] 모바일 상단 앱 바의 `CampusLog AI` 전체 문구를 유지하면서 `CampusLog`를 PC 메뉴와 같은 브랜드 워드마크체로 통일하고 390px 무겹침·44px 높이 유지 (`ISSUE-212`)
- [x] CampusLog AI 추천 헤더의 모바일 `추천 기록` 액션에 18px History 아이콘과 문구를 함께 표시하고 390px 제목 무겹침·44px 높이 유지 (`ISSUE-211`)
- [x] CampusLog AI 추천 화면 설명을 `지원 문항·JD에 맞는 경험을 찾아보세요.`로 간결하게 바꿔 449px·390px 모바일에서 한 줄 유지 (`ISSUE-210`)
- [x] 나의 활동 모바일 완료 경험 상세의 비대칭 우측 여백을 제거하고 `상세 보기` 전체 폭·`수정/삭제` 2열·AI 분석 전체 폭의 좌우 대칭 액션 배치 적용 (`ISSUE-209`)
- [x] 모바일 상단 앱 바의 축약 메뉴를 원래 제목 `오늘의 기록 / 나의 활동 / CampusLog AI`로 복원하고, CampusLog 브랜드 서체는 후속 `ISSUE-212`에서 PC와 통일 (`ISSUE-208`)
- [x] 오늘의 기록 캘린더 모바일 헤더에서 이전·오늘·다음 control group을 연·월 Select와 같은 행의 우측 상단으로 이동하고 390px 가로 overflow 없이 44px 터치 높이 유지 (`ISSUE-207`)
...
- [x] CampusLog AI route skeleton의 Grid stretch를 제거하고 최종 페이지와 Breadcrumb·헤더·우측 액션·본문 시작선 일치 (`ISSUE-196`)
- [x] CampusLog AI가 서버 RLS 경험 개수 판별로 빈 상태형·활동 보유 폼형 스켈레톤을 새로고침 첫 프레임부터 정확히 선택 (`ISSUE-195`)
- [x] CampusLog AI 새로고침 스켈레톤을 완료 빈 상태 탭과 같은 위치·폭·215px 높이로 맞춰 로딩 전후 크기 점프 제거 (`ISSUE-194`)
- [x] 전체 화면용 Liquid Skeleton 프리셋을 구현하고, 후속 검토에서 시각 적용 범위를 세 주 화면으로 축소 (`ISSUE-193`, `ISSUE-199`)
- [x] 나의 활동 완료 경험 상세의 첨부 구획을 관련 링크와 같은 평평한 section·행 밀도로 통일 (`ISSUE-192`)
- [x] 경험 폼 활동기간 그룹 제목과 시작·종료 보조 label의 크기·색·굵기 위계를 분리 (`ISSUE-191`)
- [x] 새 경험 폼의 다중 행 역할·활동 내용·첨부 간격·관련 링크·단색 파일/저장 액션·hover를 정리하고 작성/수정 스크롤 프레임을 상세와 통일 (`ISSUE-190`)
- [x] 나의 활동 빈 상태를 패널 남은 높이에 맞추고 중복 `+`를 제거하며 과거 경험 이동을 Glass capsule로 통일 (`ISSUE-189`)
- [x] 오늘의 기록 활동 현황 빈 상태를 진행 활동 기준으로 정리하고 데이터 로딩 실패를 재시도 가능한 Glass 팝업으로 전환 (`ISSUE-188`)
- [x] 로그인·회원가입을 현대적인 공통 Liquid Glass 카드로 통일하고 회원가입의 빈 방식 선택 버튼을 제거 (`ISSUE-187`)
- [x] 활동 추가의 전체 화면 확장 surface를 제거하고 작은 중앙 탭 motion으로 축소하며 날짜별 기록과 90% 밝은 Glass 색을 통일 (`ISSUE-186`)
- [x] 활동 추가 ExpandableScreen과 날짜별 기록 FloatingPanel을 쿨 뉴트럴 단일 Glass·16px 입력·capsule 액션으로 고도화 (`ISSUE-185`)
- [x] 새로고침 시 CSS가 빠지는 손상된 개발 캐시를 보존 이동하고 localhost:3000 서버 재시작·연속 새로고침 검증 (`ISSUE-184`)
- [x] 삭제 버튼의 글자·아이콘을 같은 빨강으로 통일하고 작은 액션의 과한 표면 그림자를 얕은 공통 그림자로 개선 (`ISSUE-183`)
- [x] 삭제 액션과 colorful AI 분석 실행의 의미 효과를 유지한 채 공통 44px capsule geometry 적용 (`ISSUE-182`)
...
- [x] 추천 기록을 나의 활동과 같은 primary-page·차콜 검색·master-detail·transient scrollbar 구조로 통일하고 우측 상세의 중복 결과 kicker 제거 (`ISSUE-177`)
- [x] 독립·스플릿 AI 분석 결과 헤더의 중복 `AI 경험 분석 결과` 문구를 제거하고 생성일·대표 제목 위계만 유지 (`ISSUE-176`)
- [x] 나의 활동 목록→독립 경험 상세·AI 분석의 Breadcrumb·H1·패널 시작선을 통일하고 스크롤 전체 길이·gutter·track inset을 안정화 (`ISSUE-175`)
- [x] 독립 경험 상세·AI 분석의 마지막 액션 아래 고정 30px 안전 여백을 확보해 하단 곡률 잘림 제거 (`ISSUE-174`)
- [x] 독립 경험 상세·AI 분석의 도입부와 패널을 분리한 채 함께 스크롤하고, 패널 하단은 항상 좌측 메뉴 하단에 고정하며 최종 상단도 좌측 메뉴에 정렬 (`ISSUE-173`)
- [x] 독립 경험 상세·AI 분석의 내부 스크롤바를 외곽 곡률에서 분리하고 하단 정렬·버튼 모양·상단 복귀·중복 구분선을 통일 (`ISSUE-172`)
- [x] 독립 완료 경험 상세의 폭을 공통 콘텐츠 레일까지 넓히고 이중 여백을 제거하며 데스크톱 하단을 좌측 메뉴와 정렬 (`ISSUE-171`)
- [x] 독립 완료 경험 상세에 나의 활동 공통 페이지 H1·설명을 추가하고 경험명을 카드 내부 H2로 정리 (`ISSUE-170`)
- [x] 나의 활동 진행 중 상세의 누락된 내부 스크롤·안전 여백을 복원하고 날짜별 기록 `+`를 활동 추가와 같은 차콜 원형 CTA로 통일 (`ISSUE-169`)
- [x] 활동 현황·활동 상세·나의 활동 진행 목록/상세의 상태색 workflow capsule을 통일하고 AI 초안·경험 수정 폼의 중복 선·설명·legend 정렬 정리 (`ISSUE-168`)
- [x] 나의 활동 AI 분석 패널 하단을 좌측 메뉴와 정렬하고 외곽 Glass와 내부 transient scroll viewport를 분리하며 scrollbar 위·아래 30px 안전 여백 적용 (`ISSUE-167`)
- [x] 오늘의 기록 활동 현황을 상태·기간·채움 pin·행 메뉴·고정 5행 페이지네이션이 있는 반응형 Liquid Glass Data Grid로 통합 (`ISSUE-166`)
- [x] 나의 활동 첫 화면의 전체 활동 단독 패널과 열린 목록·상세 패널 하단을 좌측 메뉴 하단과 통일 (`ISSUE-165`)
- [x] 활동 상세의 긴 설명을 상단 액션 영역에서 분리하고 흰색 요약 패널의 첫 행으로 이동 (`ISSUE-164`)
- [x] 오늘의 기록에서 진행 활동 상세로 이동할 때 과거 크림색 shimmer 로딩 카드를 제거하고 접근성 안내만 유지 (`ISSUE-163`)
- [x] 이미지 단독 자기소개서 추천 결과의 OCR 전체 문장 제목을 60자 이하 추출 의도·핵심 키워드·고정 기본 제목 순서로 정리하고 원 질문·텍스트 및 혼합 입력·긴 JD 계약 유지. 사용자 직접 로직 테스트 완료 (`ISSUE-202`)
- [x] 오늘의 기록 Data Grid 압정을 별 즐겨찾기와 분리된 사용자별 `dashboard_activity_pins` 저장·상단 정렬·실패 rollback 흐름으로 구현하고 원격 Supabase migration·REST schema 확인 완료 (`ISSUE-201`, 로그인 UI smoke test 남음)
...
- [x] 나의 활동 기본 목록 패널을 오늘의 기록·추천 입력과 같은 공통 콘텐츠 폭으로 확장 (`ISSUE-157`)
...
- [x] 자기소개서 답변 생성에 100~2000자 직접 입력을 추가하고 목표 제한·생성 범위를 API 검증, 저장 결과, 스트리밍 화면에 일관되게 반영. 사용자 직접 로직 테스트 완료 (`ISSUE-142`)
- [x] 기존 AI 작업 중 UI를 유지하면서 내부 route 이동 중 경험 분석·추천·활동 종료 합성·답변 초안 요청을 계속하고 compact 진행·완료·실패 알림과 중복 요청 방지를 제공. 사용자 직접 로직 테스트 완료 (`ISSUE-143`)
- [x] 백그라운드 AI 추천 완료 알림에서 저장된 추천 id를 사용해 추천 기록의 정확한 결과 상세로 바로 이동. 사용자 직접 로직 테스트 완료 (`ISSUE-144`)
- [x] 240자를 넘는 JD 원문을 추천 결과 제목에서 분리하고 짧은 분석 제목·기본 접힘 원문·모바일 제한 높이로 추천 기록 가독성을 개선하고 사용자 직접 로직 테스트 완료 (`ISSUE-145`)
...
- [x] 답변 생성 목적별 제한: 추천 v2 선택 경험 기반 자기소개서 300자 / 500자 / 1000자와 100~2000자 직접 입력, 면접 30초 / 1분 이상 / 예상 꼬리 질문, JD 지원 전략, 기타 맞춤 결과 단일 초안 schema / 저장 / 표시 구현 (`ISSUE-079`, `ISSUE-142`)
