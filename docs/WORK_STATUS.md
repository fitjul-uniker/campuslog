...
2026-07-09 기준으로 `docs/DESIGN.md`와 `docs/IMPLEMENTATION_PLAN.md`를 확정하고, `feature/base-structure` 작업을 통해 `web/`의 Next.js App Router 기본 구조를 `main`에 머지했습니다. 현재 앱에는 메인 / 경험 작성 / 경험 상세 / 경험 수정 / AI 분석 결과 / AI 추천 placeholder 라우트, AppShell, Navigation, 공통 스타일, 기본 타입 파일이 있습니다. 실제 경험 CRUD, localStorage 저장, AI API Route 연결은 아직 구현 전입니다.
