...
현재 프로젝트는 `feature/ai-analysis` 브랜치에서 AI 경험 분석 API Route, 분석 요청 CTA, 분석 결과 저장 / 조회 화면 구현을 완료하고 검증 중인 상태입니다. `feature/base-structure`와 `feature/experience-crud`는 `main`에 merge된 상태이며, `feature/ai-analysis`는 아직 PR 생성 / merge 전입니다. 다음 구현 단계는 `feature/ai-recommendation` 범위의 AI 경험 추천 및 활용 기능입니다.
...
현재 구현은 `feature/ai-analysis` 단계입니다. 경험 저장 / 수정 / 삭제와 AI 경험 분석 요청 / 결과 저장은 localStorage와 Next.js API Route 기준으로 구현되어 있습니다. AI 경험 추천 요청은 아직 실제 API와 연결되지 않았습니다.

## 아직 시작하지 않은 작업

- AI 경험 추천 및 활용 화면 구현
  - 추천 결과 localStorage 저장 구현 (첫 구현에서는 복사 기능보다 낮은 2순위)
- 정렬 / 필터 선택 UI와 실제 동작 구현
- README 실행 방법 보완
- Vercel 배포 준비
...
- [ ] `feature/ai-analysis` PR 생성, 리뷰, merge
- [ ] 유효한 API Key 기준으로 브라우저에서 AI 분석 성공 후 localStorage 저장 / 목록 `분석 완료` / 수정 후 `재분석 필요` / 다시 분석 성공 흐름 추가 확인
- [ ] `feature/ai-recommendation` 브랜치에서 `/api/recommend` 구현
- [ ] AI 경험 추천 및 활용 화면과 추천 결과 저장 흐름 구현
- [ ] Vercel 배포 준비
