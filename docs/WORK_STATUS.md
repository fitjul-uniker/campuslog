...
- [x] 팀 리뷰와 main merge
- [x] 2차 MVP 기능 브랜치 시작
- [x] 오늘의 기록 활동 현황의 모바일 카드 변환을 제거하고 데스크톱과 같은 표 머리글·열·62px 행을 유지하며 Grid 내부에만 가로 스크롤 적용 (`ISSUE-215`)
- [x] 백그라운드 AI compact pending 상태에서 경험 분석·추천·활동 합성·답변 초안의 작업별 문구를 2.4초마다 순환하고 실제 서버 상태만 라이브 영역으로 전달 (`ISSUE-214`)
- [x] 독립 AI 분석의 헤더 hairline→첫 경험 요약 제목 간격을 46px에서 22px로 줄여 이후 구획과 통일 (`ISSUE-213`)
- [x] 나의 활동 완료 경험 상세의 역할 제목·본문 간격, 굵기, 글자 크기, 행간과 최대 폭을 활동 내용·성과와 통일 (`ISSUE-212`)
- [x] 독립 AI 분석 결과의 헤더 하단 1px hairline만 유지하고 첫 경험 요약 상단선을 모든 화면 폭에서 제거 (`ISSUE-211`)
- [x] 독립 AI 분석 헤더의 `나의 활동으로 돌아가기`를 제거하고 `활동 경험 상세` 단일 복귀 액션을 기존 데스크톱 위치와 모바일 H1 우측 상단에 유지 (`ISSUE-210`)
- [x] 독립 완료 경험 상세의 모바일 `나의 활동` 복귀 액션을 H1 우측 상단에 유지하고 550px·390px 상세 Glass 무겹침·44px 높이·가로 overflow 없음 확인 (`ISSUE-209`)
- [x] 모바일 상단 앱 바의 `CampusLog AI` 전체 문구를 유지하면서 `CampusLog`를 PC 메뉴와 같은 브랜드 워드마크체로 통일하고 390px 무겹침·44px 높이 유지 (`ISSUE-208`)
- [x] CampusLog AI 추천 헤더의 모바일 `추천 기록` 액션에 18px History 아이콘과 문구를 함께 표시하고 390px 제목 무겹침·44px 높이 유지 (`ISSUE-207`)
- [x] CampusLog AI 추천 화면 설명을 `지원 문항·JD에 맞는 경험을 찾아보세요.`로 간결하게 바꿔 449px·390px 모바일에서 한 줄 유지 (`ISSUE-206`)
- [x] 나의 활동 모바일 완료 경험 상세의 비대칭 우측 여백을 제거하고 `상세 보기` 전체 폭·`수정/삭제` 2열·AI 분석 전체 폭의 좌우 대칭 액션 배치 적용 (`ISSUE-205`)
- [x] 모바일 상단 앱 바의 축약 메뉴를 원래 제목 `오늘의 기록 / 나의 활동 / CampusLog AI`로 복원하고, CampusLog 브랜드 서체는 후속 `ISSUE-208`에서 PC와 통일 (`ISSUE-204`)
- [x] 오늘의 기록 캘린더 모바일 헤더에서 이전·오늘·다음 control group을 연·월 Select와 같은 행의 우측 상단으로 이동하고 390px 가로 overflow 없이 44px 터치 높이 유지 (`ISSUE-203`)
