...
현재 프로젝트는 `feature/ai-recommendation` 브랜치에서 AI 경험 추천 API Route, 추천 입력 화면, 추천 결과 표시 / 복사 / 저장, 추천 기록 별도 화면 구현을 완료하고 검증 중인 상태입니다. `feature/base-structure`와 `feature/experience-crud`는 `main`에 merge된 상태이며, AI 분석과 AI 추천 기능은 1차 MVP 핵심 흐름 기준으로 코드 구현이 완료되었습니다. 다음 단계는 유효한 API Key 기준 전체 브라우저 시나리오 검증과 Vercel 배포 준비입니다.
...
현재 구현은 `feature/ai-recommendation` 단계입니다. 경험 저장 / 수정 / 삭제, AI 경험 분석 요청 / 결과 저장, AI 경험 추천 요청 / 결과 저장은 localStorage와 Next.js API Route 기준으로 구현되어 있습니다.
...
- [ ] `feature/ai-recommendation` PR 생성, 리뷰, merge
- [ ] 유효한 API Key 기준으로 브라우저에서 경험 생성 -> AI 분석 성공 -> `/recommend` 추천 성공 -> `/recommend/history` 추천 기록 유지 / 열람 흐름 확인
- [ ] Vercel 배포 준비
