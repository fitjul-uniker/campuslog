...
| ISSUE-024 | Open | High | Risk | 공개 배포에서 AI API Route 반복 호출로 OpenAI 비용이 증가할 수 있음 | `/api/analyze`, `/api/recommend`, `/api/synthesize-activity`의 로그인 세션 확인, 입력 상한, rate limit, `retryAfter`, 중복 요청, OpenAI spend limit / alert 정책 결정 | 로그인·DB foundation 확인 후 다음 최우선 개발 항목으로 전환. 입력 상한, timeout, API Key 서버 보관은 적용했으나 서버리스 환경의 신뢰 가능한 호출 빈도 제한과 UI 오류 contract는 아직 필요 | Track A + Track B | 2026-07-13 | - |
...
| ISSUE-031 | Open | High | Decision | 목적 / JD 원문 / 지원 질문 기반 AI 추천, 부족 경험 비교, 답변 초안 범위 확정 필요 | 입력 유형, JD / 질문 parser, OCR / vision 처리 방식, 원본 이미지 저장 여부, 요구사항 추출 schema, 보유·부족 경험 비교 기준, 답변 초안 저장 여부, 비용 제한 결정 | 로그인·DB foundation 확인 후 AI 추천 고도화 단계로 전환. 추천 고도화는 AI API 보호와 AI 분석 품질 개선 이후 진행. OCR 이미지는 우선 원본 저장 없이 일회성 AI 입력으로 처리하고 Storage 도입은 별도 결정 | Track A + Track B | 2026-07-13 | - |
...
| ISSUE-034 | Open | High | Decision | AI 분석 품질 개선과 근거 구조 contract 필요 | 경험 요약, 역량, 성과, 키워드, 원본 근거 연결, 재분석 필요 상태, model / prompt / schema version 기록 범위 결정 | 로그인·DB foundation 확인 후 AI API 보호 다음 작업으로 진행. 분석 결과는 기록에 없는 사실을 만들지 않고 추천 고도화가 재사용할 수 있는 구조를 우선한다 | Track A + Track B | 2026-07-13 | - |
