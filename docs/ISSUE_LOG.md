...
| ISSUE-025 | In Progress | High | Decision | v1.1 localStorage 데이터를 2차 MVP 사용자 DB로 이전할 정책 필요 | 이전 UX 문구, 충돌 선택 화면, 실제 upsert 구현과 재시도 화면 확정 필요 | 자동 이전·자동 삭제 금지, 사용자 확인 후 이전, 원본 보존, 동일 local id 기반 멱등 upsert, 샘플·fixture·파싱 실패 제외 사유 제공, 부분 실패·재시도 기록, migration ledger 테이블을 기본 정책으로 `docs/DATA_CONTRACT.md`에 정리 | Track A + Track B | 2026-07-13 | - |
