...
- 적용 상태: `main`에 반영된 PR #30 + AI 분석 v2 확장 + 추천 v2 확장 + `feature/ai-answer-drafts` 확장
- 범위: 사용자별 Supabase Postgres schema, RLS, localStorage 모델 매핑, repository 경계, 주요 UI의 Supabase repository 전환, localStorage 자동 이전·자동 삭제 금지 정책, AI 경험 분석 v2 저장 호환성, 추천 v2 저장 호환성, 답변 초안 v1 저장 호환성
...
- 원본에 없는 사실을 보강하기 위한 별도 사용자 답변 저장 위치는 아직 정하지 않았고, 기록 보완 루프에서 결정합니다.
...
- 답변 초안 본문은 추천 row를 확장하지 않고 별도 `AnswerDraftResult`로 저장합니다. 보완 질문과 사용자 보완 답변 저장 위치는 아직 정하지 않았고 기록 보완 루프에서 결정합니다.
