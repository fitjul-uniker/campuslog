...
- 적용 상태: `main`에 반영된 PR #30 + AI 분석 v2 확장 + 추천 v2 확장 + 답변 초안 v1 확장 + `feature/ai-evidence-followup` 확장
- 범위: 사용자별 Supabase Postgres schema, RLS, localStorage 모델 매핑, repository 경계, 주요 UI의 Supabase repository 전환, localStorage 자동 이전·자동 삭제 금지 정책, AI 경험 분석 v2 저장 호환성, 추천 v2 저장 호환성, 답변 초안 v1 저장 호환성, 기록 보완 질문 / 답변 저장 호환성
...
| `ExperienceFollowup` | `campuslog:v1:experience-followups` | `ExperienceFollowup[]` | 원본 경험을 자동 수정하지 않는 별도 보완 질문 / 답변 저장소. `experienceId`로 `Experience.id`를 참조하며 답변 저장 시 기존 분석이 있던 경험은 `needs_reanalysis`로 표시 |
...
| `experience_analyses` | `ExperienceAnalysis` | `user_id` | `(user_id, id)`, unique `(user_id, experience_id)` | 경험별 최신 분석 1개를 upsert. v2 분석은 `schema_version`, `prompt_version`, `model`, `star`, `evidence`, `evidence_gaps`, `cover_letter_angles`, `competency_evidence`를 함께 저장 |
...
- v2 분석은 STAR 구조, 원본 근거, 부족 정보, 자소서 소재 각도, 역량별 근거를 추가로 저장합니다.
...
- 보완 답변은 `experience_followups` / `campuslog:v1:experience-followups`에 별도로 저장하고 원본 `Experience.description` / `achievements`를 자동 수정하지 않습니다.
- `/api/analyze` 재분석은 원본 경험과 answered followup을 함께 사용하며, 보완 답변 기반 evidence는 `source: "followupAnswers"`로 구분합니다.
...
- `ExperienceFollowup`은 `schemaVersion`, `experienceId`, `source`, optional `sourceRecommendationId`, optional `sourceAnswerDraftType`, `questions`, `answers`, `status`, `generatedAt`, `updatedAt`을 저장합니다.
...
- 보완 답변이 저장되고 기존 분석이 있던 경험은 `analysisStatus: "needs_reanalysis"`로 표시합니다. 분석 재생성 성공 후 repository가 기존 계약대로 `analysisStatus: "analyzed"`로 되돌립니다.
- 기존 추천 / 답변 초안 row는 보완 답변으로 자동 재생성하지 않습니다. 화면에서는 해당 경험이 `needs_reanalysis`인 경우 기존 추천 / 초안이 stale일 수 있음을 안내합니다.
...
- 기록 보완 migration `20260714000400_experience_followups.sql`은 Supabase project 적용 후 로그인 세션에서 질문 생성, 답변 저장, 재분석 stale 표시 smoke test가 필요합니다.
