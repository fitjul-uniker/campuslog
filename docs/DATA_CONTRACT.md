...
- 적용 상태: `main`에 반영된 PR #30 + AI 분석 v2 확장 + `feature/ai-recommendation-v2` 확장
- 범위: 사용자별 Supabase Postgres schema, RLS, localStorage 모델 매핑, repository 경계, 주요 UI의 Supabase repository 전환, localStorage 자동 이전·자동 삭제 금지 정책, AI 경험 분석 v2 저장 호환성, 추천 v2 저장 호환성
- 기준: v1.1 UI/UX와 기존 도메인 타입을 유지하고, DB 전환은 작은 PR로 단계적으로 진행
- 제외: 300자 / 700자 / 면접 / 포트폴리오 답변 초안, 기록 보완 루프, OCR
...
- 답변 초안 본문, 보완 질문, 사용자 보완 답변 저장 위치는 아직 정하지 않았고 후속 답변 초안 / 기록 보완 루프에서 결정합니다.
