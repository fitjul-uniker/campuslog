...
| Test ID | 검증 결과 | 증거 |
| --- | --- | --- |
| TC-001 | Node 테스트 전체 통과 | 최신 182 tests, 182 pass, 0 fail, 약 944ms |
...
- 관련 회귀: profile action 구조 테스트 추가, 전체 Node 182개·lint·typecheck·production build 모두 PASS
...
- 우선순위 1 — `TC-028`: 로그인 상태의 malformed JSON·본문 상한·최소 근거·rate limit. OpenAI 호출 전에 차단되는 입력부터 확인하면 추가 토큰 없이 보호 경계를 검증할 수 있다.
- 우선순위 2 — `TC-022`: 실제 AI 분석 1회와 저장·새로고침. 핵심 기능이지만 추가 유료 호출이므로 별도 승인된 호출 예산에서 수행한다.
- 우선순위 3 — `TC-023`: AI 실패·취소 시 원본과 마지막 유효 결과 보존. live 실패를 억지로 만들기보다 fault injection 또는 mock 가능한 통합 harness를 먼저 준비하는 편이 안전하다.
- 우선순위 4 — `TC-027`: 답변 초안 실제 생성·분량·누적 저장. 분석·추천 기반선 이후 대표 분량 1회만 호출하는 것이 효율적이다.

## 최종 상태

`COMPLETE_WITH_NOT_RUN`

핵심 사용자 흐름, 자동화 기준선과 production build, 로그인·RLS, 실제 DB 경험·링크·private PDF 업로드·개별 삭제, 활동·날짜별 기록의 생성·수정·개별 삭제, OpenAI 자기소개서 추천 1회와 자동 저장, preview·실데이터 모바일 검증을 완료했습니다. 빈 QA 계정에 만든 합성 데이터는 최종적으로 활동·기록·경험·첨부·추천 기록 모두 0개로 정리했습니다. 추가 OpenAI 호출 또는 별도 fault-injection 환경이 필요한 4개 항목만 `NOT_RUN`으로 남겼으며 실행 가능한 항목에 `FAIL`이나 `BLOCKED`는 없습니다.
